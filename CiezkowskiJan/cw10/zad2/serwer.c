...
void message_handler(msg_buffer* msg, client* client) {
    ...
    switch(type) {
        case LIST:
            ...
            new_msg.type = LIST;
            sendto(client->fd, &new_msg, sizeof(new_msg), 0, (struct sockaddr*)&client->addr, client->addrlen);
            break;
        case TOALL:
            ...
            for (int i = 0; i < MAX_NO_CLIENTS; i++) {
                if (clients[i].is_active == 1) {
                    sendto(clients[i].fd, &new_msg, sizeof(new_msg), 0, (struct sockaddr*)&clients[i].addr, clients[i].addrlen);
                }
            }
        ...
        case TOONE:
            ...
            strcpy(new_msg.mtext, msg->mtext);
            new_msg.type = TOONE;
            sendto(rfd, &new_msg, sizeof(new_msg), 0, (struct sockaddr*)&client->addr, client->addrlen);
...
void signal_handler(int signo) {
    ...
    for (int i = 0; i < MAX_NO_CLIENTS; i++) {
        if (clients[i].is_active != -1) {
            sendto(clients[i].fd, &msg, sizeof(msg), 0, (struct sockaddr*)&clients[i].addr, clients[i].addrlen);
            close(clients[i].fd);
        }
    }
    ...
    while(1) {
        ...
        for (int i = 0; i < no_events; i++) {
            if (events[i].data.fd == local_d || events[i].data.fd == web_d) {
                ...
                union addr addr;
                socklen_t addrlen;
                recvfrom(new_fd, &msg, sizeof(msg), 0, (struct sockaddr*)&addr, &addrlen);
                for (int j = 0; j < MAX_NO_CLIENTS; j++) {
                    ...
                    if (clients[j].is_active == -1) {
                        ...
                        if (check_name_validity(msg.mtext)) {
                            ...
                            clients[j].fd = new_fd;
                            clients[j].addr = addr;
                            clients[j].addrlen = addrlen;
                            msg.type = HELLO;
                            char new_log[200];
                            sprintf(new_log, "TIME: %d:%d:%d\nACTION: SIGN IN\nFROM: %s\n\n", msg.time_struct.tm_hour, msg.time_struct.tm_min, msg.time_struct.tm_sec, clients[j].name);
                            fwrite(new_log, sizeof(char), strchr(new_log, '\0') - new_log, logs);
                            sendto(clients[j].fd, &msg, sizeof(msg), 0, (struct sockaddr*)&addr, addrlen);
                        }
                        else {
                            msg.type = TAKEN;
                            sendto(new_fd, &msg, sizeof(msg), 0, (struct sockaddr*)&addr, addrlen);
                        }
                        pthread_mutex_unlock(&mutex);
                        break;
                    }
