...
void connect_local(const char* path, sa_family_t family) {
...
void connect_web(const char* ip, in_port_t port, sa_family_t family) {
    fd = socket(family, SOCK_STREAM, 0);
...
int main(int argc, char** argv) {
    ...
    char* address = argv[3];
    if (sin_family == AF_UNIX) connect_local(address, sin_family);
    else {
        if (argc != 5) {
            fprintf(stderr, "invalid number of arguments\n");
            return 5;
        ...
        connect_web(address, port, sin_family);
    }
